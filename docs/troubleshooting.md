...
- Hotkey does nothing: another app may own the hotkey. Open Settings and choose a different hotkey, then run Doctor.
...
- Browser UI does not open: port `8765` may be busy. Tray and hotkey dictation can still work. Quit and relaunch Local Dictation, then try `Open Localhost GUI` again.
...
The browser UI at `http://127.0.0.1:8765/` is available while the tray app is running and shows the current state, last dictation result, settings path, and core setup status.
